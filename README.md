...
- **v1.7.0** — Retrospective-driven methodology 개선 4종 (todo-app 예제 회고 결과 반영):
  ...
  - **A4 (Med)**: `templates/reports/plan/plan.md` 의 "반대 의견 보존" 을 YAML 블록 규약으로 구조화. `resolved_by: open | retro-YYYY-MM-DD | v<semver> | out_of_scope` 필드로 **다음 retro 에서 종결 추적 가능**. 보존만 되고 묻히는 영구 보류 문제 해소.
  - **A3 (Low)**: `examples/todo-app` 에 vitest + @testing-library/react 테스트 추가 — `lib/storage.test.ts` 7건 + `lib/use-todos.test.ts` 9건 = **16 green**. E1-S3 AC-3(깨진 JSON fallback) 과 `loadedRef` regression guard 테스트 포함. 이 예제 초기 출시(v1.6.2) 때 반대 의견으로 보존됐던 항목 종결.
  - 예제 측 변경: `examples/todo-app/docs/retrospectives/retro-todo-app.actions.md` 의 A1~A4 전부 `✅ done` · `종결 근거: v1.7.0`. [examples/todo-app/README.md](examples/todo-app/README.md) 에 테스트 실행 가이드 추가.
