...
플래그(`--since=...`)는 무시하고 첫 위치 인자만 `scripts/resolve-plan-path.sh` 로 넘긴다.
...
Plan의 담당 영역에 따라 추가: `backend`, `frontend`, `data`, `qa` 등
...
**완료된 Task**:
- ✅ E1-S1-T1: users 컬럼 마이그레이션 (data)
- ✅ E1-S1-T2: tokens 테이블 생성 (data)

**진행 중 → 완료**:
- ✅ E1-S1 (Story 완료) — 담당 data
...
|----|------|------|------|------|---------------|
| E2-S1-T3 | SES 프로덕션 access | backend+platform | AWS 승인 대기 | 2일차 | **YES** |
